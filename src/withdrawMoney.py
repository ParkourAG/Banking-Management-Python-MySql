...
def transaction_debit_update(messageLabel, acc_id, ammount):
    db=db_connect()
...
def withdrawMoney(root):
    ...
    tk.Entry(root, width=30).pack(pady=(5,10))
    ...
    tk.Button(
        root,
        text="Withdraw Money",
        command=lambda:transaction_debit_update(messageLabel, acc_entry.get(), money.get())
    ).pack(pady=10)
