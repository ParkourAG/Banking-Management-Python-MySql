...
from db_operations import isAdminExist, isAdminBlocked


def block_account(messageLabel, admin_id, root):
    try:
        db= db_connect()
...
#  Check if account exists
        if isAdminExist(db, admin_id):
            ...
            if isAdminBlocked(db, admin_id)==False:
                cursor=db.cursor()
                sql= f"UPDATE employees \
                        SET admin_status = 'blocked' \
                        WHERE emp_id = {admin_id};"
                cursor.execute(sql)
                db.commit()

                # printing message
                messageLabel.config(text=f"Admin Id no:{admin_id} is blocked successfully.")
            else:
                # printing message
                messageLabel.config(text=f"Admin Id no:{admin_id} is already blocked.")
...
def blockAdmin(root):
    ...
    tk.Label(
        header,
        text="Block Administrator Account",
        font=("Segoe UI", 11),
        bg="#0f4c81",
        fg="#dbeafe"
    ).pack()
